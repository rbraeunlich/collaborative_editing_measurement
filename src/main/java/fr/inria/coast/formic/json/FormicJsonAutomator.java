 ...
 *
 */
public class FormicJsonAutomator extends CollaborativeAutomator {

	//redefine setting
	//protected String REMOTE_ADDR[] = {"152.81.15.203","152.81.3.91","152.81.15.71","152.81.12.192"};
	//protected int REMOTE_THREAD[] = {15,15,10,10};

	public FormicJsonAutomator(int n_user, int type_spd, int exp_id,
		...
		this.reader = new FormicJsonReader(n_user, type_spd, docUrl, exp_id, textSize);
		String formicStringId = ((FormicJsonReader)reader).getStringId();
		this.writer = new FormicJsonWriter(n_user, type_spd, docUrl, exp_id, formicStringId, textSize);
		if (this.n_user > 1) {
			this.dummies = new FormicJsonDummyWriter [n_user - 1];
			for (int i = 0; i < n_LocalThread - 1; i++) {
				this.dummies [i] = new FormicJsonDummyWriter(n_user, type_spd, docUrl, exp_id, formicStringId) ;
			}
			if (n_user > threshold) {
				this.remoteDummies = new FormicJsonRemoteDummyWriter [n_user - threshold];
				for (int i = 0; i < n_user - threshold; i++) {
					...
					this.remoteDummies [i] = new FormicJsonRemoteDummyWriter(n_user, type_spd, docUrl, exp_id, remoteURL, formicStringId);
				}
			}
		}
	}
